...
Constructs api_response.json — the mock backend payload for the React dashboard.
Sources:
...
# ── Load source files ──────────────────────────────────────────────────────────
with open('/home/ubuntu/latest_snapshot.json') as f:
    snapshot_raw = json.load(f)

with open('/home/ubuntu/correlation_history.json') as f:
    history_raw = json.load(f)

with open('/home/ubuntu/morning_briefing.json') as f:
    briefing_raw = json.load(f)

with open('/home/ubuntu/macro_regimes_kb.json') as f:
    kb_raw = json.load(f)
...
# Sort each pair's records by date ascending
...
for p in snapshot_raw['pairs']:
    ...
    # Build historical_analogs list with full detail
    analogs = []
...
# ── Section 5: heatmap_matrix (for the correlation heatmap component) ──────────
# Build a 6x6 symmetric matrix of 30d correlations for the heatmap
pair_ids = [p['pair_name'] for p in snapshot_raw['pairs']]
corr_map = {p['pair_name']: p['corr_30d'] for p in snapshot_raw['pairs']}

# For the heatmap we expose individual asset-level 30d correlations
# We list each pair's corr_30d in a flat structure the frontend can pivot
heatmap_data = {
...
output_path = '/home/ubuntu/api_response.json'
...
for alert in regime_alerts:
    print(f"    [{alert['alert_severity'].upper()}] {alert['pair_id']}  gap={alert['regime_shift_gap']}  analogs={len(alert['historical_analogs'])}")
